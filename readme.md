...
```bash
$ curl https://github.com/prometeo-cloud/prometeo/blob/master/docker-build.sh && sh docker-build.sh
```
[Docker Compose](https://docs.docker.com/compose/) needs to be installed so that the following command can be run:

```bash
$ curl https://github.com/prometeo-cloud/prometeo/blob/master/docker-compose.yml && docker-compose up
```
